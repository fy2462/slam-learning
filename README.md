...
1.data目录添加problem-be-op数据文件，文件格式如下：
    * 第一行为相机数量、路标数量、总观测数量。
    * 第二行为第i个相机观测第j个路标所看到的像素坐标。
    * 后半部分为相机的内外、外参、畸变共9维信息和路标的3维空间值的观测值。
2. include/src目录下添加beop、ceres、flags、tools目录，其中：
    * beop 包括后端g2o优化代码和输入数据处理代码。
    * ceres 是一个非线性的优化库，我们使用优化库中Differentiate方法来自动获得雅可比矩阵。
    * flags 是处理命令标签的工具类代码。
    * tools 是需要使用的数学工具代码。
3. 更新了g2o_types.h/g2o_types.cpp文件，添加了自定义的g2o的两个节点和对应的二元边。
    * VertexCameraBAL 为9维的相机优化顶点。
    * VertexPointBAL 为3维的空间点优化顶点。
    * EdgeObservationBAL 为连接两类顶点的二元边。
    对computeError和operator方法的重写，实现对雅可比和误差的计算。

### 数据读取和写入

1. 对数据文件进行解析，将数据文件中的相机欧拉角数据转换为四元数位姿，路标点解析3D位置坐标，
2. 相机颜色设定为(0,255,0)，路标颜色设定为(255,255,255)，并保存到initial.ply文件中。
3. 对读入数据进行正则化，这样会加速梯度下降运算速度。
4. 优化后的结果写入final.ply文件，格式与initial.ply一致。

### g2o优化

g2o优化部分与VO部分类似，核心方法为BuildProblem：
1. 使用相机位姿和路标3D坐标作为优化顶点，关联至边后进行优化。
2. 在添加路标点时，需要设置setMarginalized为true，已开启Schur功能。
Schur是利用雅可比矩阵的稀疏性质，对矩阵进行变换，会使计算量大大降低，并简化计算步骤。
3. 如果设置了robustify标着，那么会引入鲁棒核函数，该核函数会收敛误差较大数据到阀值边界，
从而缩小大误差对整体结果的影响。
4. 优化进行20轮，过程类似下图所示：

![alt tag](./image/opt_result.jpg)

### 优化前后对比

下图为数据优化前后对比，可见图优化的准确度和效率都是很高的。

优化前：

![alt tag](./image/initial_pc.jpg)

优化后：

![alt tag](./image/final_pc.jpg)

## 总结

本项目是对SLAM视觉里程计和后端优化的学习实践工程，然后SLAM是一个复杂、惊喜的工程，
以上两个方向只是完成了一小部分的学习工作，以后会对更多的工程问题进行分析、实践，完善工程代码。
对于SLAM中的回环检测、建图等方向还有待去研究和实践，今后会继续学习SLAM的各方面知识。

最后，送给自己一句诗，勉励自己继续前行：**雄关漫道真如铁,而今迈步从头越**